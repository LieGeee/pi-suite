...
test("toggles the diff panel from the keyboard shortcut and renders changed files on the right", async () => {
  ...
  try {
    ...
    await expect(diffPanel.locator(".diff-panel__title")).toContainText("检查面板");
    ...
    await expect(window.locator(".center-diff-preview .diff-inline")).toBeVisible();
    await expect(window.locator(".center-diff-preview .diff-line--added")).toHaveCount(1);
