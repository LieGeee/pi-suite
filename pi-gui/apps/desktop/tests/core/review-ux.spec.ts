...
test("syntax-highlights known languages and leaves unknown extensions plain", async () => {
  ...
  try {
    ...
    const tsDiff = window.locator(".center-diff-preview .diff-inline");
    ...
    await expect(window.locator(".center-diff-preview .diff-inline")).toHaveAttribute("data-language", "python");
    await expect(window.locator('.center-diff-preview .diff-inline [class*="hljs-"]').first()).toBeVisible();
    await expect(diffPanel.locator(".diff-inline")).toHaveCount(0);

    const mdRow = diffPanel.locator('.diff-panel__file[data-file-path="notes.md"]');
    await mdRow.locator(".diff-panel__file-name").click();
    const mdDiff = window.locator(".center-diff-preview .diff-inline");
...
test("reviewed checkboxes update counter, prune on changes, and survive relaunch", async () => {
  ...
  await firstWindow.getByLabel("切换改动面板").click();
  ...
  await expect(counter).toHaveText("已审查 0 / 3");

  await diffPanel.getByTestId("diff-panel-reviewed-src/foo.ts").check();
  await expect(counter).toHaveText("已审查 1 / 3");
  ...
  await expect(counter).toHaveText("已审查 2 / 3");
  ...
  try {
    ...
    await expect(reopenedPanel.getByTestId("diff-panel-counter")).toHaveText("已审查 2 / 3");
    ...
    await expect(reopenedPanel.getByTestId("diff-panel-counter")).toHaveText("已审查 1 / 2");
...
test("view-in-changes button on a write tool row opens the diff panel without toggling expand", async () => {
  ...
  try {
    ...
    await expect(window.locator(".center-diff-preview .diff-inline")).toHaveAttribute("data-language", "typescript");
    await expect(diffPanel.locator(".diff-inline")).toHaveCount(0);
...
test("highlighting tokens swap palettes when the dark class flips", async () => {
  ...
  try {
    ...
    await diffPanel
      ...
      .click();
    const token = window.locator('.center-diff-preview .diff-inline [class*="hljs-"]').first();
