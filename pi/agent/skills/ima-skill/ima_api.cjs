...
const DEFAULT_AGENT_DIR = process.env.PI_CODING_AGENT_DIR || path.resolve(__dirname, '..', '..');
const DEFAULT_CONFIG_DIR = path.join(DEFAULT_AGENT_DIR, 'config', 'ima');
const DEFAULT_LAST_CHECK_FILE = path.join(DEFAULT_CONFIG_DIR, 'last_update_check');
...
function resolveBaseUrl(options = {}) {
  const configuredBaseUrl = options.baseUrl || process.env.IMA_BASE_URL || DEFAULT_BASE_URL;
  let parsed;

  try {
    parsed = new URL(configuredBaseUrl);
  } catch {
    throw new Error(`IMA only supports ${DEFAULT_BASE_URL} as its API Base URL.`);
  }

  const isOfficialBaseUrl =
    parsed.protocol === 'https:' &&
    parsed.hostname === 'ima.qq.com' &&
    !parsed.port &&
    !parsed.username &&
    !parsed.password &&
...
function loadCredentials(options = {}) {
  const clientId =
    ...
    readFileSafe(path.join(DEFAULT_CONFIG_DIR, 'client_id'));
  const apiKey =
    options.apiKey ||
    process.env.IMA_API_KEY ||
    process.env.IMA_OPENAPI_APIKEY ||
    readFileSafe(path.join(DEFAULT_CONFIG_DIR, 'api_key'));

  if (!clientId || !apiKey) {
    const err = new Error('missing credentials');
    err.code = ERR_PROGRAMMATIC;
    err.msg =
      `未找到 IMA 凭证（clientId / apiKey）。请设置 IMA_CLIENT_ID 和 IMA_API_KEY 环境变量，或将凭证放置在 ${DEFAULT_CONFIG_DIR} 目录下。`;
    throw err;
  }
