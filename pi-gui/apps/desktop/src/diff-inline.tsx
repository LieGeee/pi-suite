...
export function InlineDiff({
  ...
  const lines = useMemo(() => parseDiffForRenderer(diff), [diff]);
...
export function parseDiffForRenderer(diff: string, maxLines = 1_200): DiffLine[] {
  ...
  for (const line of lines) {
    if (result.length >= maxLines) {
      result.push({ type: "header", content: `...[diff 过长，界面仅渲染前 ${maxLines} 行]...` });
      break;
    }
    if (line.startsWith("@@")) {
