...
import { appendTerminalReplay, appendTerminalReplayChunks } from "./terminal-model";
...
export function TerminalPanel({
  ...
  const pendingReplayChunksRef = useRef(new Map<string, string[]>());
  const replayFlushFrameRef = useRef<number | null>(null);
  ...
  const flushPendingReplayChunks = useCallback(() => {
    replayFlushFrameRef.current = null;
    const pending = pendingReplayChunksRef.current;
    if (pending.size === 0) {
      return;
    }
    pendingReplayChunksRef.current = new Map();
    setPanel((currentPanel) => {
      let nextPanel = currentPanel;
      for (const [terminalId, chunks] of pending) {
        nextPanel = updateSession(nextPanel, terminalId, (session) => ({
          ...session,
          ...appendTerminalReplayChunks(session.replay, chunks, session.truncated),
        }));
      }
      return nextPanel;
    });
  ...
  const scheduleReplayFlush = useCallback(() => {
    if (replayFlushFrameRef.current !== null) {
      return;
    }
    replayFlushFrameRef.current = window.requestAnimationFrame(flushPendingReplayChunks);
  }, [flushPendingReplayChunks]);
  ...
  useEffect(() => {
    ...
    const removeData = api.onTerminalData((event) => {
      const chunks = pendingReplayChunksRef.current.get(event.terminalId) ?? [];
      chunks.push(event.data);
      pendingReplayChunksRef.current.set(event.terminalId, chunks);
      scheduleReplayFlush();
    ...
    return () => {
      ...
      removeError();
      if (replayFlushFrameRef.current !== null) {
        window.cancelAnimationFrame(replayFlushFrameRef.current);
        replayFlushFrameRef.current = null;
      }
      pendingReplayChunksRef.current.clear();
    };
  }, [api, flushPendingReplayChunks, scheduleReplayFlush]);
