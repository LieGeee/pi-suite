...
test("settings lets the user save an API 密钥 for a built-in provider", async () => {
  ...
  try {
    ...
    await window.getByRole("button", { name: "提供商", exact: true }).click();
    await expect(window.locator(".view-header__title")).toHaveText("提供商");

    const allProviders = window.locator(".settings-section", {
      has: window.locator(".settings-section__title", { hasText: "全部提供商" }),
    });
    ...
    await dialog.getByLabel("openai API 密钥").fill("test-openai-key");
