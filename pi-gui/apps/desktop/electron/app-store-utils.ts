...
export function buildWorkspaceRecords(
  ...
  const sessionsByWorkspace = new Map<string, SessionCatalogEntry[]>();
...
export function makeTranscriptMessage(role: "user" | "assistant", text: string): Extract<TranscriptMessage, { kind: "message" }> {
  return {
    kind: "message",
    id: randomUUID(),
    role,
