...
test("keeps the mid-thread viewport stable when the composer grows away from the bottom", async () => {
  ...
  try {
    ...
    await seedTranscriptMessages(harness, window, {
      count: 80,
      textFactory: (index) => {
        if (index === 76) return sentinelText;
        if (index === 79) return finalText;
        return `Mid-thread seed row ${index} `.repeat(12);
      },
    });
...
test("keeps transcript pinning semantics while assistant deltas stream into the same row", async () => {
  ...
  try {
    ...
    const finalSeedMarker = "Streaming seed row 79";
    await seedTranscriptMessages(harness, window, {
      count: 80,
      textFactory: (index) => `Streaming seed row ${index} `.repeat(12),
    });
