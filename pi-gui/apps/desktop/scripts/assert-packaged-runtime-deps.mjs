...
const notificationHelperPath =
  packagePlatform === "darwin"
    ...
    : undefined;
const pnpmBinary = process.platform === "win32" ? (process.env.ComSpec || "cmd.exe") : "pnpm";
...
try {
  const asarArgs = ["exec", "asar", "extract", asarPath, extractedDir];
  execFileSync(
    pnpmBinary,
    process.platform === "win32" ? ["/d", "/s", "/c", "pnpm.cmd", ...asarArgs] : asarArgs,
    {
      cwd: desktopDir,
      stdio: "pipe",
    },
...
function cleanupExtractedDirectory(extractedDir) {
  try {
    rmSync(extractedDir, { recursive: true, force: true, maxRetries: 8, retryDelay: 250 });
  } catch (error) {
    if (process.platform !== "win32" || error?.code !== "EPERM") {
      throw error;
    }
    const escapedPath = extractedDir.replace(/'/g, "''");
    const cleanup = spawn(
      "powershell.exe",
      [
        "-NoProfile",
        "-NonInteractive",
        "-Command",
        `Start-Sleep -Milliseconds 750; Remove-Item -LiteralPath '${escapedPath}' -Recurse -Force -ErrorAction SilentlyContinue`,
      ],
