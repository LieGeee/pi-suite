...
const MAX_TERMINAL_DATA_CHUNK_LENGTH = 32 * 1024;
...
export class TerminalService {
  ...
  private spawnPty(webContents: WebContents, session: TerminalSession): void {
    ...
    session.dataSubscription = session.pty.onData((data) => {
      ...
      for (const chunk of chunkTerminalDataForRenderer(data)) {
        this.sendToOwner(webContents, session, desktopIpc.terminalData, { terminalId: session.id, data: chunk });
      }
    });
...
export function chunkTerminalDataForRenderer(
  data: string,
  chunkLength = MAX_TERMINAL_DATA_CHUNK_LENGTH,
): string[] {
  if (!data || chunkLength <= 0) {
    return [];
  }
  ...
  const chunks: string[] = [];
  for (let index = 0; index < data.length; index += chunkLength) {
    chunks.push(data.slice(index, index + chunkLength));
  }
  return chunks;
}
