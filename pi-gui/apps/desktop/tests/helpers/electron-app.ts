...
export async function resolvePackagedAppExecutable(releaseDir = packagedReleaseDir): Promise<string> {
  if (process.platform === "win32") {
    let releaseEntries;
    try {
      releaseEntries = await readdir(releaseDir, { withFileTypes: true });
    } catch (error) {
      if (isMissingPathError(error)) {
        throw new Error(
          `Packaged release directory not found: ${releaseDir}. Run pnpm --filter @pi-gui/desktop run package:dir first.`,
    ...
    for (const unpackedDir of unpackedDirs) {
      ...
      const executable = entries.find(
        (entry) => entry.isFile() && entry.name.toLowerCase() === "pi-gui.exe",
      );
      if (executable) {
    ...
    throw new Error(
      `No pi-gui.exe found under ${releaseDir}. Run pnpm --filter @pi-gui/desktop run package:dir first.`,
    );
  }
...
export async function scrollTimelineAwayFromBottom(window: Page, pixels = 160): Promise<void> {
  ...
  await window.evaluate((distance) => {
    ...
    if (pane.scrollHeight - pane.scrollTop - pane.clientHeight < distance / 2) {
      pane.scrollTop = Math.max(0, pane.scrollTop - distance);
      pane.dispatchEvent(new Event("scroll", { bubbles: true }));
    }
  }, pixels);
}
...
export async function deferNextSendUserMessage(harness: DesktopHarness): Promise<void> {
  await harness.electronApp.evaluate(async () => {
    const hooks = (globalThis as {
      __PI_APP_TEST_HOOKS?: { deferNextSendUserMessage?: () => void };
    }).__PI_APP_TEST_HOOKS;
    if (!hooks?.deferNextSendUserMessage) {
      throw new Error("Deferred send-user-message hook is unavailable");
    }
    hooks.deferNextSendUserMessage();
  });
}

export async function waitForDeferredSendUserMessageStarted(harness: DesktopHarness): Promise<void> {
  await harness.electronApp.evaluate(async () => {
    const hooks = (globalThis as {
      __PI_APP_TEST_HOOKS?: { waitForDeferredSendUserMessageStarted?: () => Promise<void> };
    }).__PI_APP_TEST_HOOKS;
    if (!hooks?.waitForDeferredSendUserMessageStarted) {
      throw new Error("Deferred send-user-message wait hook is unavailable");
    }
    await hooks.waitForDeferredSendUserMessageStarted();
  });
}

export async function resolveDeferredSendUserMessage(harness: DesktopHarness): Promise<void> {
  await harness.electronApp.evaluate(async () => {
    const hooks = (globalThis as {
      __PI_APP_TEST_HOOKS?: { resolveDeferredSendUserMessage?: () => void };
    }).__PI_APP_TEST_HOOKS;
    if (!hooks?.resolveDeferredSendUserMessage) {
      throw new Error("Deferred send-user-message resolve hook is unavailable");
    }
    hooks.resolveDeferredSendUserMessage();
  });
}
...
export async function selectSession(window: Page, sessionTitle: string): Promise<void> {
  ...
  await expect.poll(async () => {
    const state = await getDesktopState(window);
    for (const workspace of state.workspaces) {
      const session = workspace.sessions.find((entry) => entry.title === sessionTitle);
      if (session) {
        return state.selectedSessionId === session.id;
      }
    }
    return false;
  }).toBe(true);
...
export async function openNewThread(window: Page): Promise<void> {
  ...
  const button = window.locator(".sidebar").getByRole("button", { name: "新对话", exact: true });
...
export async function createSessionViaIpc(window: Page, workspaceIdOrPath: string, title: string): Promise<void> {
  ...
  await expect
    .poll(async () => {
      const state = await getDesktopState(window);
      return state.workspaces.some((workspace) => workspace.sessions.some((session) => session.title === title));
    }, { timeout: 15_000 })
    .toBe(true);
}

export async function openConversationView(window: Page): Promise<void> {
  const button = window
    .locator(".sidebar__nav--primary")
