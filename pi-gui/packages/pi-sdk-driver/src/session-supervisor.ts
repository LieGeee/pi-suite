...
import { retainPendingQueuedMessages } from "./queued-message-replacement.js";
import {
  buildSnapshot,
  createWorkspaceRef,
  deriveSessionConfig,
...
const NEW_THREAD_PLACEHOLDER_TITLE = "新对话";
...
export class SessionSupervisor {
  ...
  async reloadTranscriptFromDisk(sessionRef: SessionRef): Promise<SessionTranscriptMessage[]> {
    const record = await this.ensureRecord(sessionRef);
    ...
    if (!sessionFile) {
      throw new Error(`Session ${sessionKey(sessionRef)} cannot be reloaded because no session file is tracked.`);
    }
    ...
    record.sessionFile = runtime.session.sessionFile ?? runtime.session.sessionManager.getSessionFile();
    record.title = sessionEntry?.title ?? record.title;
    record.status = sessionEntry?.status ?? "idle";
    record.updatedAt = sessionEntry?.updatedAt ?? record.updatedAt;
    record.archivedAt = sessionEntry?.archivedAt;
    record.preview = sessionEntry?.previewSnippet ?? record.preview;
    record.config = deriveSessionConfig(runtime.session.sessionManager);
    record.closed = false;
    await this.bindSessionRuntime(record);

    return transcriptFromMessages(runtime.session.messages, record.updatedAt);
  }
  ...
  /**
   * Create a lightweight, transient session that does NOT bind extensions,
   * write to the catalog, or create persistent session files.  Intended for
   * development-mode subagents that only need a one-shot prompt → response
   * through the model without the full agent infrastructure overhead.
   */
  async createTransientSession(
    ...
    const createOptions: CreateAgentSessionOptions = {
      cwd: workspace.path,
      // In-memory session avoids disk I/O, force-persist, and catalog writes.
    ...
    const record = this.createRecord(workspace, runtime, options?.title ?? "transient");
    // Skip bindSessionRuntime — extension binding is the single most expensive
    // part of session creation and is unnecessary for transient model calls.
    // Skip persistSnapshot — no catalog entry needed.
  ...
  async replaceQueuedMessages(sessionRef: SessionRef, messages: readonly SessionQueuedMessage[]): Promise<void> {
    ...
    record.queuedMessages = retainPendingQueuedMessages(record.queuedMessages, messages)
  ...
  async deleteSession(sessionRef: SessionRef): Promise<void> {
    ...
    // Best-effort catalog removal; transient sessions have no catalog entry.
    try {
      await this.catalogs.sessions.deleteSession(sessionRef);
    } catch {
      // Ignore — catalog entry may not exist for transient sessions.
    }
  }
  ...
  private async handleAgentEvent(record: ManagedSessionRecord, event: AgentSessionEvent): Promise<void> {
    // Transient sessions have no listeners and skip persistence — skip the
    // entire event processing pipeline to avoid allocating temporary event
    // objects and scheduling microtasks.
    if (record.transient && record.listeners.size === 0) return;
    ...
    // Text deltas can arrive very frequently while streaming. Persisting the
    // session catalog snapshot for every token/fragment is expensive in the
    // Electron main process, especially when several development subagents run
    // in parallel. Keep deltas in memory and persist at coarser lifecycle
    // events such as sessionUpdated/runCompleted.
    this.queueDriverEvents(record, mapped, {
      persistSnapshot: mapped.some((mappedEvent) => mappedEvent.type !== "assistantDelta"),
    });
  }
    ...
    switch (event.type) {
      ...
      case "message_update":
        ...
        if (event.message.role === "assistant" && event.assistantMessageEvent.type === "text_delta") {
          // Text deltas can arrive many times per second. Do NOT attach a
          // companion sessionUpdated event here; that path persists catalog
          // snapshots and can block Electron's main process until output is
          // released in large bursts.
          return [{
...
function queuedMessageFromInput(input: SessionMessageInput, timestamp: string): SessionQueuedMessage {
  return {
    id: input.queuedMessageId ?? crypto.randomUUID(),
