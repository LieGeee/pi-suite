...
export class NotificationManager {
  ...
  private static readonly MAX_COMPLETED_RUN_KEYS = 1_000;
  ...
  private async handleEvent(event: SessionDriverEvent): Promise<void> {
    ...
    if (event.type === "runCompleted") {
      const dedupeKey = `${sessionKey(event.sessionRef)}:${event.runId ?? event.timestamp}`;
      if (this.completedRunKeys.has(dedupeKey)) {
        return;
      }
      this.completedRunKeys.add(dedupeKey);
      if (this.completedRunKeys.size > NotificationManager.MAX_COMPLETED_RUN_KEYS) {
        // Prune half to bound memory for long-running app sessions.
        const entries = [...this.completedRunKeys];
        this.completedRunKeys.clear();
        for (let i = Math.floor(entries.length / 2); i < entries.length; i += 1) {
          const entry = entries[i];
          if (entry) this.completedRunKeys.add(entry);
        }
      }
      await this.showNotification(event.sessionRef, event.snapshot.title, "本次处理已完成");
      return;
    }
  ...
  private titleForSession(sessionRef: SessionRef): string {
    return this.sessionFromLatestState(sessionRef)?.title ?? "pi 对话";
  }
}
