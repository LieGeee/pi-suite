...
export async function renameSession(
  ...
  if (!nextTitle) {
    return store.withError("会话名称不能为空。");
  }
...
export async function syncWorkspace(
  ...
  return store.withErrorHandling(async () => {
    const synced = await store.driver.syncWorkspace(workspace.path, workspace.name);
    const selectedSessionId =
      refreshOptions.selectedWorkspaceId === workspaceId
        ? refreshOptions.selectedSessionId
        : undefined;
    if (selectedSessionId && synced.sessions.some((session) => session.sessionRef.sessionId === selectedSessionId)) {
      await store.reloadTranscriptFromDriver(
