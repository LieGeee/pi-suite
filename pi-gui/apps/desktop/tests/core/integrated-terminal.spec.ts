...
test("opens a workspace terminal with persistent output, tabs, and takeover controls", async () => {
  ...
  try {
    ...
    await runTerminalCommand(window, "node -e \"console.log('PI_TERMINAL_OK'); console.log(process.cwd())\"");
    ...
    await runTerminalCommand(
      window,
      "node -e \"console.log(['PI_GUI_COMPUTER_USE_LOCKED_USE_APP_TOKEN','PI_GUI_COMPUTER_USE_DESKTOP_PID','PI_GUI_COMPUTER_USE_DESKTOP_PATH','PI_GUI_COMPUTER_USE_LOCKED_USE_AUTH_SOCKET'].map((name) => process.env[name] || 'missing').join(':'))\"",
    );
...
test("writes text into the integrated terminal once", async () => {
  const userDataDir = await makeUserDataDir();
  ...
  try {
    ...
    await window.keyboard.type("PI_TERMINAL_INPUT_ONCE");

    await expect
      .poll(async () => countOccurrences((await terminal.locator(".xterm-rows").innerText()) ?? "", "PI_TERMINAL_INPUT_ONCE"))
      .toBe(1);
  } finally {
    await harness.close();
  }
...
async function runTerminalCommand(window: import("@playwright/test").Page, command: string): Promise<void> {
