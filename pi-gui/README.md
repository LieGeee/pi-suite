...
- Beta (macOS arm64, Linux AppImage, Windows source builds)
- Public source repo
...
Windows release artifacts are not currently published upstream. On Windows, build from source with `pnpm --filter @pi-gui/desktop run package:win`.
...
- On Windows, this app prefers `PI_CODING_AGENT_DIR`, then `PI_GUI_AGENT_DIR`, then `S:/tool/pi/agent` when present.
