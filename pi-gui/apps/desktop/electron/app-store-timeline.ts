...
export function appendAssistantDelta(
  ...
  if (activeId) {
    ...
    const lastItem = lastIndex >= 0 ? transcript[lastIndex] : undefined;
    const index = lastItem?.id === activeId ? lastIndex : transcript.findIndex((message) => message.id === activeId);
...
export function applyTimelineEvent(
  ...
  switch (event.type) {
    ...
    case "sessionUpdated":
      if (event.snapshot.status === "running" && event.snapshot.runningRunId && !state.runningSinceBySession.has(key)) {
        ...
        const activity = makeActivityItem("处理中…");
    ...
    case "toolStarted": {
      ...
      // Update the working activity to show which tool is running and the count
      const activityId = state.activeWorkingActivityBySession.get(key);
      if (activityId) {
        const idx = transcript.findIndex((t) => t.kind === "activity" && t.id === activityId);
        if (idx >= 0) {
          const current = transcript[idx];
          if (current?.kind === "activity") {
            const toolLabelText = toolLabel(event.toolName, event.input);
            transcript[idx] = {
              ...current,
              label: `${toolLabelText} — 工具 #${metrics.toolCount}`,
            };
          }
        }
      }
    ...
    case "toolFinished":
      ...
      // Restore generic working label after tool finishes
      const activeId = state.activeWorkingActivityBySession.get(key);
      if (activeId) {
        const idx = transcript.findIndex((t) => t.kind === "activity" && t.id === activeId);
        if (idx >= 0) {
          const current = transcript[idx];
          if (current?.kind === "activity" && current.label !== "处理中…") {
            transcript[idx] = { ...current, label: "处理中…" };
          }
        }
      }
      break;
  ...
  // No need for transcriptCache.set() — we mutated the array in-place (no spread), so the Map already has the same reference.
}
