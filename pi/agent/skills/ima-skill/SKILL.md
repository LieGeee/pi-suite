...
metadata:
  ...
  security:
    credentials_usage: |
      ...
      Credentials may be read from local files under the active Pi agent config directory.
      They are never logged or transmitted to any other destination.
    allowed_domains:
      - ima.qq.com
      - '*.myqcloud.com'
---
...
| 用户意图                                                                                   | 模块           | 读取                      |
| ------------------------------------------------------------------------------------------ | -------------- | ------------------------- |
| 搜索笔记、浏览笔记本、获取笔记内容、创建笔记、追加内容                                     | notes          | `notes/MODULE.md`          |
| 上传文件、添加网页链接、搜索知识库、浏览知识库内容、获取知识库信息、获取可添加的知识库列表 | knowledge-base | `knowledge-base/MODULE.md` |
| 查看原文、分析原文、导出原文（需要 media_id）                                              | knowledge-base | `knowledge-base/MODULE.md` |
...
某些意图跨越 notes 和 knowledge-base 两个模块。**不要只读取一个子模块就开始执行**，必须先读取两个模块的 MODULE.md 再按顺序操作。

| 用户说的                             | 实际流程                                      | 读取顺序                                               |
| ------------------------------------ | --------------------------------------------- | ------------------------------------------------------ |
| "把知识库里的XX内容记到笔记"         | KB 搜索/读取 → Notes 创建/追加                | 先读 `knowledge-base/MODULE.md` → 再读 `notes/MODULE.md` |
| "查看原文"（知识库中的笔记类型媒体） | KB `get_media_info` → Notes `get_doc_content` | 先读 `knowledge-base/MODULE.md` → 再读 `notes/MODULE.md` |
| "把这篇笔记添加到知识库"             | Notes 搜索获取 note_id → KB `add_knowledge`   | 先读 `notes/MODULE.md` → 再读 `knowledge-base/MODULE.md` |
...
!`IMA_CONFIG_DIR="${PI_CODING_AGENT_DIR:?PI_CODING_AGENT_DIR is not set}/config/ima"; test -f "$IMA_CONFIG_DIR/client_id" && test -f "$IMA_CONFIG_DIR/api_key" && echo "✅ Credentials configured" || echo "⚠️ NO CREDENTIALS — setup required before any API call"`
...
```bash
IMA_CONFIG_DIR="${PI_CODING_AGENT_DIR:?PI_CODING_AGENT_DIR is not set}/config/ima"
mkdir -p "$IMA_CONFIG_DIR"
printf '%s' "your_client_id" > "$IMA_CONFIG_DIR/client_id"
printf '%s' "your_api_key" > "$IMA_CONFIG_DIR/api_key"
```
...
Agent 会按优先级依次尝试：环境变量 → `$PI_CODING_AGENT_DIR/config/ima` 配置文件。缺少凭证时，`node ima_api.cjs ...` 会以程序错误退出（`code: -100`），并在 stderr 输出对应 `msg`。
...
```bash
# Pi installation: credentials are read from the protected config files, not command-line arguments.
IMA_SKILL_DIR="${PI_CODING_AGENT_DIR:?PI_CODING_AGENT_DIR is not set}/skills/ima-skill"
IMA_ERR_FILE="S:/tool/pi/tmp/ima_err.$$"
trap 'rm -f "$IMA_ERR_FILE"' EXIT

# stdout 返回正常响应；stderr 返回结构化错误 {"code":-100|-200,"msg":"..."}
if ! resp=$(node "$IMA_SKILL_DIR/ima_api.cjs" "openapi/list_docs" '{"limit":10}' 2>"$IMA_ERR_FILE"); then
  err_json=$(<"$IMA_ERR_FILE")
  err_code=$(printf '%s' "$err_json" | node -e "const d=JSON.parse(require('fs').readFileSync(0,'utf8'));process.stdout.write(String(d.code??''))" 2>/dev/null)
  err_msg=$(printf '%s' "$err_json" | node -e "const d=JSON.parse(require('fs').readFileSync(0,'utf8'));process.stdout.write(String(d.msg??''))" 2>/dev/null)
