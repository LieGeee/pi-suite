...
export const MAX_RENDERER_COMPOSER_IMAGE_BYTES = 10 * 1024 * 1024;
...
export function canReadComposerImageFileInRenderer(file: Pick<File, "size">): boolean {
  return Number.isFinite(file.size) && file.size >= 0 && file.size <= MAX_RENDERER_COMPOSER_IMAGE_BYTES;
}

function readImageAttachmentFromFile(file: File): Promise<ComposerImageAttachment | null> {
  if (!canReadComposerImageFileInRenderer(file)) {
...
export function resolveComposerFilePath(file: File): string | null {
  return resolveFilePath(file as FileWithPath);
}
